 ...
 *  experimental purpose for now and subject to change. */
typedef struct grpc_tls_err_details grpc_tls_err_details;
...
/** A struct containing all information necessary to schedule/cancel a
    credential reload request. To help understand each fields, we mark each
    field with labels. Here are their explanations
    -------------------------label explanation----------------------------------
    ...
    - cb[USER-MANAGED][ASYNC_ONLY]: a callback-function provided for users to
      indicate an asynchronous reloading operation is complete. After calling
      this function, gRPC stack would be notified and continue rest of the
      authentication.
...
struct grpc_tls_credential_reload_arg {
  ...
  grpc_tls_err_details* error_details;
  grpc_tls_credential_reload_config* config;
  void* context;
...
/** A struct containing all information necessary to schedule/cancel a server
    authorization check request.To help understand each fields, we mark each
    field with labels. Here are their explanations:
    -------------------------label explanation----------------------------------
    [USER-MANAGED]: fields that are managed directly by the end users
    ...
    authorization.
    ----------------------label explanation ends--------------------------------
    - cb[USER-MANAGED][ASYNC-ONLY]: a callback-function provided for users to
      indicate an asynchronous reloading operation is complete. After calling
      this function, gRPC stack would be notified and continue rest of the
      authentication.
